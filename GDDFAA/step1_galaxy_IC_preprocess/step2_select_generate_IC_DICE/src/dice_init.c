...
int create_galaxy(galaxy *gal, char *fname, int info) {
  ...
  for (i = 0; i < AllVars.MaxCompNumber; i++) {
    ...
    // printf("density dice_init0: gal->comp_scale_dens[component] = %f\n", gal->comp_scale_dens[i]); //gjy add
    ...
    if (gal->comp_bool[i]) {
      ...
      if (gal->comp_radius_nfw[i] == -1.0) {
        ...
        if (i == gal->index_halo && gal->comp_cut[i] != gal->r200) {
          ...
          printf("density dice_init1(not else): \n\n\n\n\ngal->comp_scale_dens[component] = %f\n", gal->comp_scale_dens[i]); //gjy add
          printf("mass before: %f, %f, %f", gal->comp_mass[i], gal->comp_cut[i], cumulative_mass_func(gal, 2.0 * gal->comp_cut[i], i));

          gal->comp_scale_dens[i] = gal->comp_mass[i] /
              cumulative_mass_func(gal, 2.0 * gal->comp_cut[i], i);
          printf("mass after: gal->comp_scale_dens[component] = %f\n", gal->comp_scale_dens[i]); //gjy add
  ...
  if (gal->index_halo > -1) { //gjy comment: spin lambda
    gal->J200 =
