...
double v2a_r_1D_func(galaxy *gal, gsl_integration_workspace *w, int component) {
  ...
  gsl_integration_qng(&F, fabs(gal->r_sph[gal->index[tid]]),
                      fabs(gal->r_sph[gal->index[tid]]) + infinity, epsabs,
                      epsrel, &integral, &error, &neval);

  rho = density_functions_pool(
      gal, gal->r_cyl[gal->index[tid]], gal->theta_cyl[gal->index[tid]],
      gal->z[gal->index[tid]], 0, gal->comp_model[gal->selected_comp[tid]],
  ...
  case 1:
    ...
    break;
  case 2:
    // Separable differential equation term using Hansen & Moore 2006 definition
    // for the anisotropy parameter beta = -0.15-0.2*dln(rho)/dln(r)
...
static double dv2a_r_1D_func(double r_sph, void *params) {
  ...
  rho1 = density_functions_pool(
      gal, r_cyl, theta_cyl, z,
      gal->comp_jeans_mass_cut[gal->selected_comp[tid]],
      gal->comp_model[gal->selected_comp[tid]], gal->selected_comp[tid]);
  rho2 = density_functions_pool(gal, r_cyl, theta_cyl, z, 0,
...
double v2a_r_2D_func(galaxy *gal, gsl_integration_workspace *w, int component) {
  ...
  case 1:
    gsl_integration_qag(&F, fabs(gal->z[gal->index[tid]]),
                        fabs(gal->z[gal->index[tid]]) + infinity, epsabs,
                        epsrel, AllVars.GslWorkspaceSize, key, w, &integral,
                        &error);
    break;
  case 2:
    gsl_integration_qagiu(&F, fabs(gal->z[gal->index[tid]]), epsabs, epsrel,
                          AllVars.GslWorkspaceSize, w, &integral, &error);
    break;
  case 3:
    gsl_integration_qng(&F, fabs(gal->z[gal->index[tid]]),
                        fabs(gal->z[gal->index[tid]]) + infinity, epsabs,
                        epsrel, &integral, &error, &neval);
    break;
  default:
...
static double dv2a_r_2D_func(double z, void *params) {
  ...
  rho = density_functions_pool(
      gal, radius, theta, z, gal->comp_jeans_mass_cut[gal->selected_comp[tid]],
      gal->comp_model[gal->selected_comp[tid]], gal->selected_comp[tid]);
...
double v2a_theta_2D_func(galaxy *gal, double radius, double v2a_r, double v_c,
                         int component) {
  ...
  rho = density_functions_pool(gal, radius, gal->theta_cyl[gal->index[tid]], z,
                               0, gal->comp_model[component], component);
...
// Take a look to the part of the documentation explaining the velocity
...
double rho_v2a_r_2D_func(double radius, void *params) {
  ...
  rho = density_functions_pool(
      gal, radius, gal->theta_cyl[gal->index[tid]], gal->z[gal->index[tid]],
      gal->comp_jeans_mass_cut[gal->selected_comp[tid]],
      gal->comp_model[gal->selected_comp[tid]], gal->selected_comp[tid]);
...
double v2a_r_3D_func(galaxy *gal) {
  ...
  v2a_r =
      get_jeans_3D_cic(gal, fabs(gal->r_cyl[gal->index[tid]]),
                       fabs(gal->z[gal->index[tid]]), gal->vr2_tilted_mixed);
...
double v2a_z_3D_func(galaxy *gal) {
  ...
  v2a_z =
      get_jeans_3D_cic(gal, fabs(gal->r_cyl[gal->index[tid]]),
                       fabs(gal->z[gal->index[tid]]), gal->vz2_tilted_mixed);
...
double v2a_theta_3D_func(galaxy *gal) {
  ...
  v2a_theta =
      get_jeans_3D_cic(gal, fabs(gal->r_cyl[gal->index[tid]]),
                       fabs(gal->z[gal->index[tid]]), gal->vtheta2_mixed);
...
void fill_jeans_3D_grid(galaxy *gal, int component) {
  ...
  double *qprev, alpha, h1, h2, r1, r2, dr, z1, z2, z, dz, rho, rho1, rho2, p,
      f, dqdr;
    ...
    if (k == gal->ngrid_jeans[0] - 1) {
      ...
      for (n = 0; n < nsteps; n++) {
        ...
        for (j = 0; j < gal->ngrid_jeans[1]; j++) {
          ...
          gal->x[gal->index[tid]] = gal->r_cyl[gal->index[tid]] *
                                    cos(gal->theta_cyl[gal->index[tid]]);
          gal->y[gal->index[tid]] = gal->r_cyl[gal->index[tid]] *
                                    sin(gal->theta_cyl[gal->index[tid]]);
          gal->z[gal->index[tid]] = z;

          h1 = jeans_3D_h_func(gal, r1, z, component);
          rho = density_functions_pool(gal, r1, 0., z,
                                       gal->comp_jeans_mass_cut[component],
          ...
          if (j == gal->ngrid_jeans[1] - 1) {
            ...
            dr = r2 - r1;
            dqdr =
                (h2 * qprev[j + 1] - h1 * qprev[j]) / dr + qprev[j] * h1 / r1;
          }
          gal->vz2_tilted_mixed[k][j] = qprev[j] + (-p - dqdr) * (dz / nsteps);
        }
        for (j = 0; j < gal->ngrid_jeans[1]; j++)
          qprev[j] = gal->vz2_tilted_mixed[k][j];
      }
    }
  }
  ...
  for (k = 0; k < gal->ngrid_jeans[0]; k++) {
    for (j = 0; j < gal->ngrid_jeans[1]; j++) {
      ...
      rho = density_functions_pool(gal, r1, 0., z1,
                                   gal->comp_jeans_mass_cut[component],
                                   gal->comp_model[component], component);
      // sigma_z^2
      gal->vz2_tilted_mixed[k][j] =
          rho > 0. ? gal->vz2_tilted_mixed[k][j] / rho : 0.;
      // <v_z.v_r>
      vrvz = h1 * gal->vz2_tilted_mixed[k][j];
      // sigma_r^2
      vr2 = (2.0 * h1 / tan(2.0 * alpha) + 1.0) * gal->vz2_tilted_mixed[k][j];
      // <v_r^2>
      vr2_tilted = vr2 * pow(cos(alpha), 2) +
                   2 * vrvz * sin(alpha) * cos(alpha) +
                   gal->vz2_tilted_mixed[k][j] * pow(sin(alpha), 2);
      // <v_z^2>
      vz2_tilted = vr2 * pow(sin(alpha), 2) -
                   2 * vrvz * sin(alpha) * cos(alpha) +
                   gal->vz2_tilted_mixed[k][j] * pow(cos(alpha), 2);
  ...
  for (k = gal->ngrid_jeans[0] - 1; k >= 0; k--) {
    for (j = 0; j < gal->ngrid_jeans[1]; j++) {
      ...
      rho1 = density_functions_pool(gal, r1, 0., z1,
                                    gal->comp_jeans_mass_cut[component],
                                    gal->comp_model[component], component);
      ...
      rho2 = density_functions_pool(gal, r2, 0., z1,
                                    gal->comp_jeans_mass_cut[component],
                                    gal->comp_model[component], component);
      double vtheta2 = 0.;
      if (rho1 > 0.) {
        vtheta2 = gal->vr2_tilted_mixed[k][j] + r1 * fz +
                  r1 / rho1 *
                      (rho2 * gal->vr2_tilted_mixed[k][j2] -
                       rho1 * gal->vr2_tilted_mixed[k][j]) /
                      (r2 - r1) +
                  r1 / rho1 *
                      (rho2 * jeans_3D_h_func(gal, r2, z, component) *
                           gal->vz2_tilted_mixed[k][j2] -
                       rho1 * jeans_3D_h_func(gal, r1, z1, component) *
                           gal->vz2_tilted_mixed[k][j]) /
                      (r2 - r1);
      }
      if (vtheta2 > 0) {
...
double jeans_3D_h_func(galaxy *gal, double r, double z, int component) {
  ...
  alpha = atan2(z, r);
  f = gal->comp_jeans_f_sigma[component];
  h = ((f - 1.0) / 2.0 * tan(2 * alpha)) /
      (pow(cos(alpha), 2) - f * pow(sin(alpha), 2) +
       (1.0 + f) / 2.0 * sin(2 * alpha) * tan(2 * alpha));
...
double get_jeans_3D_cic(galaxy *gal, double r, double z, double **array) {
  ...
  if (node_r >= 0 && node_r < gal->ngrid_jeans[0] - 1 && node_z >= 0 &&
      node_z < gal->ngrid_jeans[1] - 1) {
...
double v2a_r_toomre(galaxy *gal, double radius, double v2a_r, int component) {
  ...
  // We take into account the contribution of the gas to the stability of the
  // system
  surface_density = surface_density_func(
      gal, radius, gal->theta_cyl[gal->index[tid]], 1, component);
  if (gal->comp_type[component] == 0) {
    Q = sqrt(gal->comp_cs_init[component] * fabs(kappa_sqrd)) /
        (pi * G * surface_density);
  } else {
  ...
  if (Q < gal->comp_Q_lim[component] && gal->comp_Q_lim[component] > 0) {
    v2a_r_new =
        pow(gal->comp_Q_lim[component] * 3.36 * G * surface_density, 2.0) /
        (fabs(kappa_sqrd));
    Q = gal->comp_Q_lim[component];
  }
  if (gal->comp_Q_fixed[component] > 0) {
    v2a_r_new =
        pow(gal->comp_Q_fixed[component] * 3.36 * G * surface_density, 2.0) /
        (fabs(kappa_sqrd));
    Q = gal->comp_Q_fixed[component];
  }

  gal->comp_Q_min[component] = (Q < gal->comp_Q_min[component] && Q > 0.0)
                                   ? Q
                                   : gal->comp_Q_min[component];
...
// direction for the a given flat component using the axisymmetric drift
// approximation
...
double v2_theta_gas_func(galaxy *gal, double radius, double z, int component) {
  ...
  pressure_force =
      radius * (pow(gal->comp_cs_init[component], 2.0) * density_derivative) /
      density_wrapper_func(radius, gal);
  v2_theta_gas = v_c2 + pressure_force;
  if (v2_theta_gas < 0)
    v2_theta_gas = 0.;
  if (radius > gal->comp_cut[component] + 3 * gal->comp_sigma_cut[component])
    v2_theta_gas = 0.;
    ...
    if (gal->comp_type[i] == 0) {
      if (gal->pseudo[tid]) {
        rho += pseudo_density_gas_func(
            gal, fabs(radius), gal->theta_cyl[gal->index[tid]],
            gal->z[gal->index[tid]], 0,
            gal->comp_model[gal->selected_comp[tid]], gal->selected_comp[tid],
...
// at a point with a cylindrical radius r. This is used to calculate the
// velocity dispersion.
//
...
// This function calculates the force on a test particle due to the disk
// at a point with an height z. This is used to calculate the velocity
// dispersion.
//
// This function simply performs a five point differentiation around
...
double galaxy_zforce_func(galaxy *gal, double z) {
  ...
  h = get_h_value(gal, gal->x[gal->index[tid]], gal->y[gal->index[tid]], z, 0,
                  0);
...
// This function calculates the force on a test particle due to the global
// potential at a spherical radius r_psh. This is used to calculate the velocity
// dispersion.
//
// This function simply performs a five point differentiation around
...
double galaxy_rsphforce_func(galaxy *gal, double r_sph) {
  ...
  h = get_h_value(gal, gal->x[gal->index[tid]], gal->y[gal->index[tid]],
                  gal->z[gal->index[tid]], 0, 0);
...
// This function computes the total angular momentum of a given component using
// the particles //gjy comment: angular moment and spin lambda
double Jtot_func(galaxy *gal, int component) {
  ...
  for (i = gal->comp_start_part[component];
       ...
       i++) {
    if (gal->r_sph[i] <= gal->r200) { //gjy comment: ??
      Jx += gal->mass[i] *
            (gal->y[i] * gal->vel_z[i] - gal->z[i] * gal->vel_y[i]);
      Jy += gal->mass[i] *
            (gal->z[i] * gal->vel_x[i] - gal->x[i] * gal->vel_z[i]);
      Jz += gal->mass[i] *
            (gal->x[i] * gal->vel_y[i] - gal->y[i] * gal->vel_x[i]);
    }
  }
