...
DEBUG_PRINT_I(11);
	...
	//:: what to run
	int is_writeSnapshot 		= atoi(argv[6]);	//write snapshots //0
	...
	int N_ptcs 					= atoi(argv[17]); 	//the max index of particles, value -1 is as NumPart or N_allPtcs
	...
	int snapshot_previous   = atof(argv[29]);
	bool is_record_tau_ptau = atoi(argv[30]); //0 for all particles //1 for debug some particles
DEBUG_PRINT_I(12);
...
DEBUG_PRINT_I(4);
	...
	// DEBUG_PRINT_I(442);
			...
			Plummer FPot_1(conv_G*Mass_vir, scale_length_comp[0], 0.9, 0.8);
...
DEBUG_PRINT_I(51);
		...
		////DEBUG //begin DEBUG
		if(0){
			double GM = conv_G*GA.M_scale_comp[icmp];
		...
		*/
		...
		// NFW FPot2(conv_G*vecNFW[0], vecNFW[1]/3., vecNFW[2], vecNFW[3]);
		Isochrone FPot2(conv_G*vecNFW[0], vecNFW[1], vecNFW[2], vecNFW[3]);
...
DEBUG_PRINT_I(9908);
		if(1){ //Cannot use foci method of TACT because orbit wrong shape in data potential
			// AA_TF_DP.fillDeltagrids(Delta_lmn_name); //TACT
			AA_TF_DP.readDeltagrids(Delta_lmn_name); //y_cut
		}
		...
		// Read on rank 0, then broadcast to all ranks
		std::string previous_actions_file =
		    path_gm_1 + "aa/snapshot_" + to_string(snapshot_previous) + ".action.method_all.txt";
		bool is_exist_previous_actions_file = false; // global flag: file exists + has at least one good row
		std::vector<std::vector<double>> vecAlphaBeta;
		double* inputAlphaBeta = nullptr;
		double inputAlphaBeta_buf[2] = {0.0, 0.0};

		if(1){
			int exist_flag = 0;

			if (my_rank == 0) {
				std::ifstream fin(previous_actions_file);
				if (fin.good()) {
					exist_flag = 1;
				} else {
					std::cerr << "No existing actions file " << previous_actions_file << std::endl;
					exist_flag = 0;
				}
				fin.close();
			}

			MPI_Bcast(&exist_flag, 1, MPI_INT, 0, MPI_COMM_WORLD);
			is_exist_previous_actions_file = (exist_flag != 0);

			// if (is_exist_previous_actions_file) {
			// 	std::ifstream fin(previous_actions_file);
			// 	std::string line;
			// 	vecAlphaBeta.resize((size_t)pSTAGE->SS[s]->NumPart);

			// 	while (std::getline(fin, line)) {
			// 		// ONLY skip comment lines starting with "#"
			// 		if (!line.empty() && line[0] == '#') continue;

			// 		// Parse tokens as strings so "-inf"/"inf" won't break the stream.
			// 		// Also stop when hitting inline "#..." (like "#endl").
			// 		auto parse_token = [](const std::string& s) -> double {
			// 			// handle inf/nan in various cases
			// 			if (s == "inf" || s == "+inf" || s == "INF" || s == "+INF" ||
			// 				s == "infinity" || s == "+infinity" || s == "INFINITY" || s == "+INFINITY") {
			...
			// 				return std::stod(s);
			// 			} catch (...) {
			// 				// keep row count; if malformed token appears, store NaN and continue
			// 				return std::numeric_limits<double>::quiet_NaN();
			// 			}
			// 		};

			// 		double last3 = std::numeric_limits<double>::quiet_NaN();
			// 		double last2 = std::numeric_limits<double>::quiet_NaN();
			// 		double last1 = std::numeric_limits<double>::quiet_NaN();
			// 		int nnum = 0;
			// 		int ptc_ID = 0; // all ptc_ID is data[:, 6]

			// 		std::istringstream iss(line);
			// 		std::string tok;
			// 		while (iss >> tok) {
			// 			if (!tok.empty() && tok[0] == '#') break;  // inline comment => stop reading numbers

			// 			double val = parse_token(tok);
			// 			// rolling window of last 3 numeric tokens
			// 			last3 = last2;
			// 			last2 = last1;
			// 			last1 = val;
			// 			++nnum;
			...
			// 		// python analog: data[:, -3] is alpha, data[:, -2] is beta
			// 		double alpha = last3;
			// 		double beta  = last2;

			// 		// IMPORTANT: keep every non-comment line (even if malformed) as one entry
			// 		vecAlphaBeta[ptc_ID] = {alpha, beta};
			// 		// DEBUG_PRINT_V0d(1, cols.size(), "cols.size()");
			// 		// DEBUG_PRINT_V0d(1, alpha, "alpha");
			// 		// DEBUG_PRINT_V0d(0, beta, "alpha");
			// 	}
			// 	fin.close();
			// }
			...
			if (is_exist_previous_actions_file) {
				...
				// initialize to {0,0} for all particles
				vecAlphaBeta.assign((size_t)pSTAGE->SS[s]->NumPart, std::vector<double>{0.0, 0.0});
				...
				const int NumPart = (int)pSTAGE->SS[s]->NumPart;
				...
				while (std::getline(fin, line)) {
					...
					while (iss >> tok) {
						if (!tok.empty() && tok[0] == '#') break;  // inline comment => stop

						double val = parse_token(tok);
						// column 6 (0-based) => 7th numeric token
						// treat ID token as integer, but it might appear as "1" or "1.000000e+00"
						if (nnum == 6) {
							// round-to-nearest integer is safer than cast
							ptc_ID = (int)std::llround(val);
						}
						// rolling window of last 3 numeric tokens
						last3 = last2;
						last2 = last1;
					...
					// Need at least 7 numeric tokens to have an ID, and at least 3 tokens to have last3/last2
					if (ptc_ID < 0) {
						// out-of-range ID, exit
						std::cerr<<"Wrong particle ID of actions file, please check. Exit.\n";
						exit(1);
					}

					vecAlphaBeta[ptc_ID-1][0] = last3;
					vecAlphaBeta[ptc_ID-1][1] = last2;
				}
				fin.close();
			}
		}
		std::cout<<"is_exist_previous_actions_file: "<<is_exist_previous_actions_file<<"\n";
...
DEBUG_PRINT_I(52);
			...
			for(int i=my_from;i<my_endout;i++)
			{
				...
				if((bool)B2){
					...
					// If we have a per-particle (alpha,beta) table, use it; otherwise fall back to Delta(E).
					if (is_exist_previous_actions_file && snapshot_previous>=0 && snapshot_previous!=snapshot) {
						inputAlphaBeta_buf[0] = vecAlphaBeta[ID-1][0];
						inputAlphaBeta_buf[1] = vecAlphaBeta[ID-1][1];
						inputAlphaBeta = inputAlphaBeta_buf;
						Alpha = inputAlphaBeta_buf[0];
						Beta  = inputAlphaBeta_buf[1];
					} else {
						inputAlphaBeta = nullptr;
					}
					std::cout<<"AlphaBeta: "<<Alpha<<" "<<Beta<<"\n";
					Value_Actions_TF_DP = AA_TF_DP.actions(xv0, inputAlphaBeta);
					if (!(is_exist_previous_actions_file && snapshot_previous>=0 && snapshot_previous!=snapshot)){
						Alpha = AA_TF_DP.ABC_presentCalculation[0]; //it has been changed in AA_TF_DP.actions()
						Beta = AA_TF_DP.ABC_presentCalculation[1]; 
					}
					Gamma = Gamma; //to record focus

					if(is_record_tau_ptau){
						Actions_TriaxialStackel_Fudge AA_TF_DP_tau_ptau(pDPot, Alpha, Beta);
						AA_TF_DP_tau_ptau.reset_AA_integrating_data();
						AA_TF_DP_tau_ptau.is_record_action_integrand = 1;
DEBUG_PRINT_I(6631);

						AA_TF_DP_tau_ptau.actions(xv0); //repeat as AA_TF_DP.actions() with same foci
						for(int swit_AD = 0;swit_AD<3;swit_AD++){ //tau = lambda, mu, nu
							int count_AD = AA_TF_DP_tau_ptau.PAD[swit_AD].size();
							DEBUG_PRINT_V0d(1, count_AD, "count_AD");
							vector<struct data_debug> vdd_AD;
							vdd_AD.resize(count_AD);
							VecDoub tau_i = AA_TF_DP_tau_ptau.CS->xv2tau(xv0);
							for(int i_AD=0;i_AD<count_AD;i_AD++){ //wrong i make cannot Pot.Phi() when gether
								auto AD = AA_TF_DP_tau_ptau.PAD[swit_AD][i_AD];
								vdd_AD[i_AD].xv = xv0; //index 0
								...
								vdd_AD[i_AD].value_double.push_back(tau_i[0]); //index 13
								vdd_AD[i_AD].value_double.push_back(tau_i[1]);
								vdd_AD[i_AD].value_double.push_back(tau_i[2]);
							}
							string filename_debug_AD = path_gm_1
								+"debug/tau_ptau_snapshot_"+to_string(snapshot)+"_particle_"+to_string(ID)+"_swit_"+to_string(swit_AD)+".txt";
...
DEBUG_PRINT_I(7);
		...
		// // ---- (4) potential compare, one line version -------------------------
		// // Ray through (0,0,0) and (4,2,1), plus 3^3 nearby points per radius.
		// //
		// // Output structure keeps compatibility as much as possible:
		// //   xv[0:3] = sampled (x,y,z) with offsets
		// //   xv[3]   = L (distance parameter along the ray, kpc)
		// //   xv[4]   = base index i (0..x_count-1)  [grouping key]
		// //   xv[5]   = off_id (0..26)               [which offset]
		// //
		// // value_double keeps the same 16 numbers as before:
		// //   inertia frame: SCF (P,Fx,Fy,Fz), DS (P,Fx,Fy,Fz)
		// //   rotating frame: SCF (P,Fx,Fy,Fz), DS (P,Fx,Fy,Fz)
		
		// for(int j=0;j<1;j++){ // keep loop, but write 1 file
		// 	int x_count = 100;
		// 	double x_min = 1e-2, x_max = 1e3;
		// 	VecDoub xlist = GJY::gen_logspace1D(x_min, x_max, x_count);

		// 	// direction from (0,0,0) to (4,2,1)
		// 	VecDoub dir = {4.0, 2.0, 1.0};
		// 	double dn = std::sqrt(dir[0]*dir[0] + dir[1]*dir[1] + dir[2]*dir[2]);
		// 	if(dn <= 0.0) dn = 1.0;
		// 	dir[0] /= dn; dir[1] /= dn; dir[2] /= dn;

		// 	// offset size: dh ~ halo softening (softening_type[1])
		// 	double soft = softening_type[1];
		// 	if(!(soft > 0.0) || !std::isfinite(soft)) soft = 1.0;
		// 	double dh = 0.1 * soft;
		// 	double dlist[3] = {-dh, 0.0, +dh};

		// 	const int n_off = 27;
		// 	vector<struct data_debug> vdd;
		// 	vdd.resize((size_t)x_count * (size_t)n_off);

		// 	double P = 0.0;
		// 	VecDoub F(3, 0.0);
		// 	VecDoub X(6, 0.0); // (x,y,z, meta1, meta2, meta3)

		// 	size_t idx = 0;
		// 	for(int i=0;i<x_count;i++){
		// 		const double L = xlist[i];
		// 		const double bx = dir[0] * L;
		// 		const double by = dir[1] * L;
		// 		const double bz = dir[2] * L;
		...
		// 					X[1] = by + dlist[b];
		// 					X[2] = bz + dlist[c];
		// 					// metadata for python grouping
		// 					X[3] = L;
		// 					X[4] = (double)i;
		// 					X[5] = (double)off_id;

		// 					vdd[idx].xv = X;
		// 					vdd[idx].value_double.clear();
		// 					vdd[idx].value_double.reserve(16);

		// 					// (1) inertia frame potential/force
		// 					pSTAGE->is_preprocess_rotation = true;
		// 					P = pSTAGE->potential_t(X, t0, 1, 0, 4);
		// 					F = pSTAGE->forces_t(X, t0, 1, 0, 4);
		// 					vdd[idx].value_double.push_back(P);
		// 					vdd[idx].value_double.push_back(F[0]);
		// 					vdd[idx].value_double.push_back(F[1]);
		// 					vdd[idx].value_double.push_back(F[2]);
		...
		// 					// (2) rotating frame potential/force
		// 					pSTAGE->is_preprocess_rotation = false;
		// 					P = pSTAGE->potential_t(X, t0, 1, 0, 4);
		// 					F = pSTAGE->forces_t(X, t0, 1, 0, 4);
		// 					vdd[idx].value_double.push_back(P);
		// 					vdd[idx].value_double.push_back(F[0]);
		...
		////(4) potential compare: three axis lines, 27 offsets per base point
		if(my_rank==0){
			printf("Calculate potential to compare (axis lines, 27 offsets).\n");

			const int x_count = 100;
			const double x_min = 1e-1;
			const double x_max = 1e3;
			const double r_mask_min = 0.5;

			const double soft = softening_type[1];
			if(!(soft > 0.0) || !std::isfinite(soft)){
				std::cerr << "Error: softening_type[1] is invalid: " << soft << "\n";
				exit(1);
			}

			const double dh = 0.5 * soft;
			const double dlist[3] = {-dh, 0.0, +dh};

			VecDoub xlist = GJY::gen_logspace1D(x_min, x_max, x_count);

			for(int j=0; j<3; j++){ // x, y, z
				std::string filename =
					path_gm_1 + "intermediate/potential_compare_axis_" +
					std::to_string(snapshot) + "_" + std::to_string(j) + ".txt";
				...
				fout << "# columns:\n";
				fout << "#  0:x 1:y 2:z 3:r_base 4:base_idx 5:off_id\n";
				fout << "#  6:P_SCF_inertia 7:Fx_SCF_inertia 8:Fy_SCF_inertia 9:Fz_SCF_inertia\n";
				fout << "# 10:P_DS_inertia 11:Fx_DS_inertia 12:Fy_DS_inertia 13:Fz_DS_inertia\n";
				fout << "# 14:P_SCF_rot     15:Fx_SCF_rot     16:Fy_SCF_rot     17:Fz_SCF_rot\n";
				fout << "# 18:P_DS_rot      19:Fx_DS_rot      20:Fy_DS_rot      21:Fz_DS_rot\n";
				...
				for(int i=0; i<x_count; i++){
					const double r_base = xlist[i];

					if(!(r_base > r_mask_min) || !std::isfinite(r_base)){
						continue;
					}
					...
					for(int a=0; a<3; a++){
						for(int b=0; b<3; b++){
							for(int c=0; c<3; c++){
								X[0] = base_xyz[0] + dlist[a];
								X[1] = base_xyz[1] + dlist[b];
								X[2] = base_xyz[2] + dlist[c];
								X[3] = r_base;
								X[4] = (double)base_kept;
								X[5] = (double)off_id;
								...
								P = pSTAGE->potential_t(X, t0, 1, 0, 4);
								F = pSTAGE->forces_t(X, t0, 1, 0, 4);
								vals[0] = P; vals[1] = F[0]; vals[2] = F[1]; vals[3] = F[2];

								P = pSTAGE->potential_t(X, t0, 1, 0, 1);
								F = pSTAGE->forces_t(X, t0, 1, 0, 0);
								vals[4] = P; vals[5] = F[0]; vals[6] = F[1]; vals[7] = F[2];
								...
								P = pSTAGE->potential_t(X, t0, 1, 0, 4);
								F = pSTAGE->forces_t(X, t0, 1, 0, 4);
								vals[8]  = P; vals[9]  = F[0]; vals[10] = F[1]; vals[11] = F[2];

								P = pSTAGE->potential_t(X, t0, 1, 0, 1);
								F = pSTAGE->forces_t(X, t0, 1, 0, 0);
								vals[12] = P; vals[13] = F[0]; vals[14] = F[1]; vals[15] = F[2];
