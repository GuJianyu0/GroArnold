...
void Actions_TriaxialStackel_Fudge::integrals(const VecDoub& tau){
	...
	//gjy note: the A_\tau, B_\tau of in Sanders and Binney (2015)
...
VecDoub Actions_TriaxialStackel_Fudge::find_limits(const VecDoub& tau){
	//gjy comment: the meaing of some variables
	...
	if(ptau2ROOT_Triax_Fudge(lambda, &RS_l)>0.0){
		while(ptau2ROOT_Triax_Fudge(laminner, &RS_l)>0.0 and (laminner+CS->alpha())>tiny_number)	laminner-=.1*(laminner+CS->alpha()); //gjy note: minus 0.1 times variance
