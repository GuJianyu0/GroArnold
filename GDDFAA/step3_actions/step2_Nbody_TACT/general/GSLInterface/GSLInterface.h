...
class root_find{
    ...
    public:
    	...
    	void bracket(){
			...
			if(xhi<xlo){double xt = xlo; xlo=xhi; xhi=xt;} //gjy note: swap when inverse sign
			double diff = (xhi-xlo)/2.;
			while(GSL_FN_EVAL(&F,xlo)*GSL_FN_EVAL(&F,xhi)>0.){ //gjy note: enlarge range when inverse sign
				xlo-=diff;xhi+=diff;
			}
		}
