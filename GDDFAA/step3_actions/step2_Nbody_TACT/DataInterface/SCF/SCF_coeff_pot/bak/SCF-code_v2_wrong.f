...
C        INCLUDE 'mpif.h'
     ...
     &         plm1m,plm2m,rhonltil,alm,blm,cosmphi1,sinmphi1
      ...
      real*8 eps_scf, r_true, rsoft !gjy add: softening for distance
      common /EPS_SCF/ eps_scf
      data eps_scf / 0.0d0 / !kpc
      ! data eps_scf / 5.0d0 / !kpc !wrong energy
...
crss     r, sin(theta), cos(theta), sin(phi), and cos(phi)
           ...
           rxyzinv = arxyzinv(k)
           rxyinv  = arxyinv(k)
         !   r = rxyz2 * rxyzinv !gjy comment: original
           r_true = rxyz2 * rxyzinv !gjy change: softening
           costh = z(k)*rxyzinv
           sinth = rxy2*rxyinv*rxyzinv
           cosmphi1 = x(k)*rxyinv
           sinmphi1 = y(k)*rxyinv
           rsoft = sqrt( r_true*r_true + eps_scf*eps_scf ) !gjy add: softening, softened radius for radial basis functions
...
crss     End modified loops for dplm calculation
         ...
         !   if(lmin.eq.0) then !gjy comment: original
         !   phinltil = 1./(1+r)
         !   else
         !   phinltil=r/(1.+r**3)
         !   endif
         !   ratio=r/(1.+r)**2
           ...
           if(lmin.eq.0) then !gjy change: softening
              phinltil = 1.d0/(1.d0 + rsoft)
           else
              phinltil = rsoft/(1.d0 + rsoft**3)
           endif
           ratio = rsoft/(1.d0 + rsoft)**2
...
C              DO 180 m=0,0
 ...
 180          CONTINUE
	...
	!       rhonltil = phinltil/(r*(1.+r)*(1.+r)*2.0d0*pi) !gjy comment: original
   ...
   !   &              (1.+r))+temp4*2.*twoalpha(l)/(1.+r)**2)
        ...
        rhonltil = phinltil/
     &    ( rsoft*(1.d0 + rsoft)*(1.d0 + rsoft)*2.d0*pi ) !gjy change: softening
        pot(k) = pot(k) + temp3*phinltil

        ar = ar + phinltil*
     &     ( -temp3*( l/rsoft - twolp1(l)/(1.d0 + rsoft) )
     &       + temp4*2.d0*twoalpha(l)/(1.d0 + rsoft)**2 )
