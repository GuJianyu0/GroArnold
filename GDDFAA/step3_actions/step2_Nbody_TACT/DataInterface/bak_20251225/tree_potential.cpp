...
void OctreePotential::build_node_(int node_id, int depth)
{
    ...
    if (count <= bucket_size_ || depth >= max_depth_ || !(node.hs > 0.0)) {
        node.is_leaf = true;
    ...
    for (int k = start; k < end; ++k) {
        const int i = perm_[k];
        const int ox = (x_[i] > node.cx) ? 1 : 0;
        const int oy = (y_[i] > node.cy) ? 1 : 0;
        const int oz = (z_[i] > node.cz) ? 1 : 0;
    ...
    // create children
    node.is_leaf = false;
    const double child_hs = 0.5 * node.hs;
    ...
    for (int o = 0; o < 8; ++o) {
        ...
        const int child_id = (int)nodes_.size();
        node.child[o] = child_id;

        Node child;
        // center shifts by +/- child_hs
        child.hs = child_hs;
        child.cx = node.cx + ((o & 1) ? child_hs : -child_hs);
        child.cy = node.cy + ((o & 2) ? child_hs : -child_hs);
        child.cz = node.cz + ((o & 4) ? child_hs : -child_hs);
        child.start = off[o];
