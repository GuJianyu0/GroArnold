...
def fitting_model_MPLF_freq():
    """
    1D-in-h model: fh_MPLF1_log10(h, ...)
    A simpler fit model.
...
#: examples
def fitting_model_SCBPL_freq():
    """
    1D-in-h model: fh_SCBPL_log10(h, ...)
    A simpler fit model.
    - combination: J\cdot\Omega via rateF1 (no args)
    - params_name left as None (auto-named later), it will auto-name ["pn1_fit", ..., "log_penalty"] after fit.
    """
...
def fh_SCBPL_log10(
    ...
    """
    Shouldered Cored Broken Power Law (SCBPL).

    f(h) = A (1+h/Js)^a (1+h/Jb)^-(a+p) [1+(h/Jt)^eta]^-q

    Parameters
    ----------
    Jamp : amplitude action scale, with A = M0 / Jamp^3
    Js   : shoulder scale
    Jb   : main break scale
    Jt   : outer taper scale
    a    : low-h shoulder/rise strength
    p    : intermediate-h declining slope
    q    : extra outer steepening
    eta  : taper sharpness; fix to 2 or 3 for stability
