...
# def axis_ratio_by_configuration_data(pos, r_bound=None):
#     """
#     Compute axis ratios from aligned particle coordinates.
#     q : ndarray, shape (3,)
#         Axis ratios (1, qy, qz), where
#             qy = sqrt(sum(y^2) / sum(x^2))
#             qz = sqrt(sum(z^2) / sum(x^2))
#     """
#     r = norm_l(pos, axis=1)
#     pos = np.asarray(pos)
#     if r_bound is not None:
#         pos = pos[r<=r_bound]
#     DEBUG_PRINT_V(0, len(r), len(pos))
#     sx = np.sum(pos[:, 0] ** 2)
#     sy = np.sum(pos[:, 1] ** 2)
#     sz = np.sum(pos[:, 2] ** 2)
#     return np.array([1.0, np.sqrt(sy / sx), np.sqrt(sz / sx)])

def axis_ratio_by_configuration_data(pos, r_bound=None):
    pos = np.asarray(pos, dtype=float)
    DEBUG_PRINT_V(1, pos[0], r_bound, "pos[0], r_bound")
    ...
    n_after = len(pos)
    DEBUG_PRINT_V(1, n_before, n_after, "n_before, n_after")

    if n_after == 0:
        raise ValueError("No particles left after applying r_bound.")
    ...
    if sx == 0:
        raise ValueError("sum(x^2) is zero, cannot compute axis ratios.")
