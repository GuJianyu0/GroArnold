...
###########################################################
# This file
# A Python3 controller that is similar to the workflow of `install_and_run/run.bash` for GDDFAA_work.
# Works best when run from the directory: GDDFAA_work/install_and_run/
...
class Paths:
    ...
    def from_base(home: Optional[Path] = None) -> "Paths":
        ...
        folder_foci_table = folder_actions / "step2_Data_TACT/DataInterface/SCF/orbitIntegSCF_adjust_a2b2/src/"
        ...
        return Paths(home, folder_main, folder_thisFile, folder_packages, folder_dependencies, folder_make_DICE,
                     ...
                     folder_simulation, folder_simu_setting, folder_simu_si, folder_gm, folder_snapshot_txt,
...
def step_prepare_IC(paths: Paths, model_name: str, tag_IC: int, is_modify_IC: int, logdir: Path):
    ...
    if tag_IC == 1:
        run_cmd(["bash", "step1_compile.bat"], cwd=paths.folder_DICE, log=logdir/"dice_compile.log")
        ...
        run_cmd(["bash", "step1_compile.bat"], cwd=paths.folder_cold, log=logdir/"cold_compile.log")
    ...
    run_cmd(["bash", "step1_compile.bat"], cwd=paths.folder_IC_trans, log=logdir/"ic_trans_compile.log")
...
def step_build_galaxy_folder(paths: Paths, model_name: str, history_tag: str, logdir: Path,
    ...
    if is_build_gmfolder == 1:
        ...
        # exact relative like run.bash: cp ../Gadget2/Gadget2 ./
        copy(paths.folder_gm/"../Gadget2/Gadget2", gm/"Gadget2")
        ...
        if (paths.folder_simu_si/"convert_to_txt.bat").exists():
            copy(paths.folder_simu_si/"convert_to_txt.bat", gm/"txt/convert_to_txt.bat")
        if (paths.folder_simu_si/"example_snapshot_DF_params_fit.xv.txt").exists():
    ...
    for i in snapshot_ids:
        ...
        try:
            ...
            if is_run_foci == 1:
                ...
                src = paths.folder_actions/"step2_Data_TACT/DataInterface/SCF/orbitIntegSCF_adjust_a2b2/src/some_lmn_foci_Pot.txt"
                dst = gm/"intermediate"/f"snapshot_{i}_lmn_foci_Pot.old.txt"
                if src.exists(): copy(src, dst)
                orbit_src_dir = paths.folder_actions/"step2_Data_TACT/DataInterface/SCF/orbitIntegSCF_adjust_a2b2/src/orbit"
...
def step_actions_calculation(paths: Paths, snapshot_ids: List[int], *, logdir: Path):
    ...
    for i in snapshot_ids:
        ...
        try:
            ...
            # run_cmd(["bash", "step1_1_compile_all.bat"], cwd=paths.folder_AA, log=logdir/f"actions_{sid}.log") #to compile the C++ code for angle-actions after some modification
...
def main():
    ap = argparse.ArgumentParser(prog="workflow_wrapper.py",
                                 description="Python controller for GDDFAA_work (translated from run.bash).")
    ...
    #Then parse the legacy files as usual
