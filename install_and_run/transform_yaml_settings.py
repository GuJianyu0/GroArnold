...
def main():
    ...
    # 2) user_settings_multi.txt
    t_start = _num(_safe_get(defaults, "preparation.t_start", 0.0))
    t_end   = _num(_safe_get(defaults, "preparation.t_end_inclusive", 1.0))
    dt_act  = _num(_safe_get(defaults, "preparation.dt_actions", 0.1))
    dt_tp   = _num(_safe_get(defaults, "preparation.dt_teppod", 0.1))
    ...
    n_mpi   = int(_safe_get(defaults, "preparation.n_mpi_gadget", 4))
    ...
    usm.append("# t_start  t_end_inclusive  dt_actions  dt_teppod  n_cut_snapshot  snapshot_to_compare  n_mpi_gadget  is_modify_IC")
    ...
    # 4) Copy run.param and IC_DICE_manucraft.params (and optional galaxy_general.config)
    settings_dir = yml_path.parent
    _copy_if_exists(settings_dir / "unified_settings.yaml", HERE / "unified_settings.yaml")
    _copy_if_exists(settings_dir / "run.param", HERE / "run.param")
    _copy_if_exists(settings_dir / "IC_DICE_manucraft.params", HERE / "IC_DICE_manucraft.params")
    _copy_if_exists(settings_dir / "galaxy_general.config", HERE / "galaxy_general.config")

    # 5) Also place per-model IC_DICE_{model}.params where DICE expects them
    ...
    for name in model_names:
        ...
        _copy_if_exists(src_dice, dst)

    # 6) Copy galaxy_general.config next to those needed by your DICE runner
    cfg_src = settings_dir / "galaxy_general.config"
    if cfg_src.exists():
        _copy_if_exists(cfg_src, IC_SETTING_DIR / "galaxy_general.config")
